...
# 5. 啟動 FastAPI Mock Server（選用，供 Server Mock 測試使用）
...
```python
# Flow 層封裝：將登入 → 加購 → 結帳組合成單一呼叫
# CheckoutFlow 需傳入四個物件：login_page、inventory_page、checkout_page、page
...
# 攔截 http://localhost/mock/payment，回傳假的支付成功 response
# 注意：必須使用絕對路徑，相對路徑會被 saucedemo context 誤解析
...
5. Install Allure CLI (Java)
    │
    ▼
6. Pre-create allure-results/ (mkdir -p，防止 plugin 靜默失敗)
7. Run Tests → allure-results/ (raw JSON) + report.xml
    │
    ▼
8. Generate Allure Report → allure-report/ (static HTML)
    │
    ▼
9. Upload Artifacts (report.xml + allure-results，保留 7 天)
...
**解法**：透過 `--headed` CLI option 動態判斷，本地看畫面、CI 自動無頭。同時加上 `_IS_CI` 雙重保護，確保 CI 環境即使誤傳 `--headed` 也永遠以 headless 模式執行。
...
```python
# 修正前（錯誤）—— 相對路徑被 saucedemo context 誤解析
fetch('/mock/payment')

# 修正後（正確）—— 絕對路徑可被 Playwright Route Mock 精確攔截
fetch('http://localhost/mock/payment')
```

---

### 7. conftest.py 本機邏輯覆蓋 CI 的 alluredir 設定

**問題**：CI 顯示「No files were found with the provided path: allure-results/」，GitHub Pages 始終顯示 README 而非 Allure 報告。

**根因**：`conftest.py` 的 `pytest_configure` hook 透過讀取 allure-pytest plugin 的內部 option `allure_report_dir` 來判斷是否為本機環境。但該 option 在某些版本的預設值為空字串而非 `None`，導致即使 CI 明確傳入 `--alluredir=allure-results`，本機時間戳路徑仍會將其覆蓋，使 `allure-results/` 資料夾從未在 CI 環境中建立。

**解法**：改用 `os.environ.get("CI") == "true"` 判斷執行環境（GitHub Actions 會自動注入此標準變數），不再依賴 plugin 內部 option 的命名與預設值行為，徹底隔離本機邏輯與 CI 邏輯。
...
def pytest_sessionfinish(session, exitstatus):
    if _IS_CI:
        return  # CI 環境跳過本機時間戳邏輯，保留 allure-results/ 原地不動
    # 以下為本機時間戳搬移邏輯...
```

**教訓**：環境判斷應依賴平台注入的標準環境變數，而非解析 CLI argv 或框架內部 option——後者的可用時機與預設值因版本而異，是不穩定的實作細節。

---

### 8. allure-results 資料夾不存在導致 plugin 靜默失敗

**問題**：CI log 顯示 `allure-results NOT found`，`pytest --alluredir=allure-results` 執行後資料夾完全不存在。

**根因**：`allure-pytest` plugin 在目標資料夾不存在時可能靜默失敗，不拋出任何錯誤，導致整個 CI 報告流程空轉。

**解法**：在 CI workflow 的 `Run Tests` step 前，加入 `mkdir -p allure-results` 確保資料夾在 plugin 嘗試寫入之前就已存在；同時在 `conftest.py` 的 `pytest_configure` 階段亦補上 `os.makedirs` 作為雙重保護。

```yaml
# .github/workflows/main.yml
- name: Run Tests
  run: |
    mkdir -p allure-results   # 預建資料夾，防止 allure-pytest 靜默失敗
    pytest --alluredir=allure-results --junitxml=report.xml -v || true
```

**教訓**：第三方 plugin 的寫入前置條件（如目錄存在）不可假設由 plugin 自行處理，應在呼叫端明確保障，讓 CI log 能反映真實的失敗原因。

---

### 9. --headed CLI option 與 pytest-playwright 內建選項衝突

**問題**：CI 執行時 pytest 在啟動階段即崩潰，`argparse.ArgumentError: conflicting option string: --headed`，導致零測試執行、allure-results 為空、報告部署失敗。

**根因**：`pytest-playwright` 插件本身內建了 `--headed` option，而 `conftest.py` 的 `pytest_addoption` 重複宣告了同名 option，pytest 的 argparse 層不允許重複，因此在 conftest 載入時直接拋出例外。

**解法**：移除 `conftest.py` 中重複的 `--headed` 宣告，直接使用 `pytest-playwright` 提供的版本。`request.config.getoption("--headed")` 的呼叫方式完全不變。

**教訓**：引入 pytest 插件前應確認其內建 CLI option 清單，避免在 `conftest.py` 重複宣告造成衝突。`pytest-playwright` 的內建 option 包含 `--headed`、`--browser`、`--slowmo` 等。
